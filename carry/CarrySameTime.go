...
var ProcessCarrySameTime = func(market, symbol string) {
	...
	if int(startTime)-tick.Ts > 500 || int(startTime)-tickRelated.Ts > 500 || model.AppConfig.Handle != `1` ||
		model.AppPause {
		util.Info(fmt.Sprintf(`error4 now:%d tick_%s delta:%d tick_%s delta:%d`,
			startTime, market, int(startTime)-tick.Ts, setting.MarketRelated, int(startTime)-tickRelated.Ts))
		return
	}
...
func placeBothOrders(market, symbol string, tick, tickRelated *model.BidAsk, accountRelated *model.Account,
	...
	model.SetCarryInfo(model.FunctionCarry, fmt.Sprintf("[搬砖参数] %s money price:%f %s:%f p1:%f "+
		"p2:%f py:%f px:%f ask:%f askRelated:%f\n", market, zFee, setting.MarketRelated, zFeeRelated,
